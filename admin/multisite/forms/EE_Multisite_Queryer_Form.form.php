            ...
            array(
                ...
                'subsections'     => array(
                    ...
                    'sql_query'     => new EE_Text_Area_Input(
                        array(
                            'html_help_text' => __('Only SELECT queries allowed (for now). Use the string "{$wpdb->prefix}" and "{$wpdb->base_prefix}" as you would normally. These strings will be replaced appropriately when querying each blog.',
                                'event_espresso'),
                            'required'       => true,
                            'default'        => 'SELECT',
                        )),
