...
class EED_Multisite extends EED_Module
{
     ...
     * `restore_current_blog` call.  Typically this should be set to true when client code is calling any EE code
     * (besides EEM models) that could be specific to the site (eg. EE_Config, Data Migrations, Messages system).
     *
     * @var bool
     */
    ...
    public static function set_hooks_admin()
    {
        ...
        if (is_multisite()) {
            add_action('network_admin_notices', ['EED_Multisite', 'check_network_maintenance_mode']);
            add_action('network_admin_notices', ['EED_Multisite', 'check_main_blog_maintenance_mode']);
            ...
            add_filter(
                ...
                ['EED_Multisite', 'check_main_blog_maintenance_mode'],
                10
            );
        }
    }
    ...
    protected static function set_hooks_both()
    {
        ...
        if (is_multisite()) {
            add_action(
                ...
                ['EED_Multisite', 'mark_blog_as_up_to_date_if_no_migrations_needed'],
                10,
                1
            );
            add_action('wp_insert_site', ['EED_Multisite', 'new_blog_created'], 10, 1);
            add_action('wp_loaded', ['EED_Multisite', 'update_last_requested']);
            add_filter('delete_blog', ['EED_Multisite', 'delete_ee_custom_tables_too'], 10, 2);
        }
    }
    ...
    public static function delete_ee_custom_tables_too($blog_id, $drop)
    {
        if ($drop) {
            ...
            if (isset($tables['Blog_Meta']) && $tables['Blog_Meta'] instanceof EE_Secondary_Table) {
                ...
                global $wpdb;
                $wpdb->delete($tables['Blog_Meta']->get_table_name(), ['blog_id_fk' => $blog_id]);
            }
    ...
    public static function check_main_blog_maintenance_mode($notice = '')
    {
        $new_notice = '';
        if (EE_Maintenance_Mode::instance()->level() == EE_Maintenance_Mode::level_2_complete_maintenance) {
            $maintenance_page_url = EE_Admin_Page::add_query_args_and_nonce([], EE_MAINTENANCE_ADMIN_URL);
            if (is_main_site()) {
                ...
                <p>'
                              . __(
                                  'Your event site is in the process of being updated and is currently in maintenance mode.  It has been bumped to the front of the queue and you should be able to have full access again in about 5 minutes.',
                                  'event_espresso'
                              )
                              . '</p>'
                              .
    ...
    public static function update_last_requested()
    {
        ...
        // other sites can do it fine in maintenance mode
        $user_agent = $_SERVER['HTTP_USER_AGENT'] ?? '';
        if (
            ! EED_Multisite::is_bot($user_agent)
            && ! defined('DOING_CRON')
        ) {
    ...
    /**
     * Callback for the WordPress switch_blog action that fires whenever switch_to_blog and restore_current_blog is
     * called. We use this to fire any resets on EE systems that are needed when the blog context changes. Important:
     * If self::do_full_reset() is called BEFORE this callback gets executed, then a full reset of EE is done.
     * Otherwise, the only thing that will happen is the `$new_blog_id` will be set on EEM_Base.  Also this method will
     * not do anything if in a WordPress installing context or if the switch is being done to the same blog as what's
     * being switched from.
     *
     ...
     * This is no longer needed because we hook into the switch_blog core WP action.  So core switch_to_blog and
     * restore_current_blog can be used and everything will flow through EED_Multisite::switch_to_blog method.
     *
    ...
    public function enqueue_scripts()
    {
        ...
        wp_register_script(
            'espresso_multisite',
            EE_MULTISITE_URL . 'scripts/espresso_multisite.js',
            ['jquery'],
     ...
     * (otherwise, if we wait, it's possible to get multiple simultaneous requests
